...
Mínimo 5 pessoas. Retorne SOMENTE o JSON, sem texto adicional.`;
  ...
  if (selEmpresa) {
    ...
    // ── Lusha enrichment states ───────────────────────────────────────────────
    const [lushaOpen,       setLushaOpen]       = React.useState(false);
    const [lushaStep,       setLushaStep]       = React.useState('idle'); // idle|loading|select|revealing|done
    const [lushaCandidates, setLushaCandidates] = React.useState([]);
    const [lushaSelected,   setLushaSelected]   = React.useState([]);
    const [lushaResults,    setLushaResults]    = React.useState([]);
    const [lushaCredits,    setLushaCredits]    = React.useState(null);
    const [lushaError,      setLushaError]      = React.useState('');
    const [empresaSupa,     setEmpresaSupa]     = React.useState(null); // {id, nome, website, dominios}
    ...
    function supaJwtFig(path, opts) {
      ...
      return fetch('https://uetltlnjmobeiunxfsqi.supabase.co'+path, Object.assign({},opts,{headers:h})).then(function(r){var ct=r.headers.get('content-type')||'';return ct.includes('json')?r.json():null;});
    }

    async function startLusha() {
      setLushaOpen(true);
      ...
      // 1. Busca empresa no Supabase para obter id e website
      ...
      var domain = '';
      if (emp && emp.website) {
        domain = emp.website.replace(/^https?:\/\//,'').replace(/\/.*/,'').replace(/^www\./,'');
      } else if (emp && emp.dominios && emp.dominios.length) {
        domain = emp.dominios[0];
      }

      // 2. Busca candidatos na Lusha
      ...
      if (resp.error || !Array.isArray(resp.contacts)) {
        setLushaError(resp.error || 'Sem candidatos. Verifique se a chave LUSHA_KEY está configurada na Vercel e se o plano inclui prospecting.');
        setLushaStep('idle');
        return;
      }
    ...
    async function revealLusha() {
      ...
      if (resp.error) {
        setLushaError(resp.error);
        setLushaStep('select');
        return;
      }
      if (resp.credits) setLushaCredits(resp.credits);
      setLushaResults(resp.results || []);
      setLushaStep('done');
    }

    async function saveToSupa(result) {
      if (!result || result.saved) return;
      var empId = empresaSupa && empresaSupa.id;
      if (!empId) { alert('Empresa não encontrada no banco. Cadastre primeiro via Radar.'); return; }
      var nome = (result.firstName+' '+result.lastName).trim();

      // Verifica duplicata por email, linkedin, ou nome+empresa
      var qParts = [];
      ...
      var existRows = [];
      if (qParts.length) {
        existRows = await supaJwtFig('/rest/v1/crm_decisores?empresa_id=eq.'+empId+'&or=('+qParts.join(',')+')&select=id&limit=1');
      }
      if (!existRows || !existRows.length) {
        // Busca por nome
        existRows = await supaJwtFig('/rest/v1/crm_decisores?empresa_id=eq.'+empId+'&nome=ilike.'+encodeURIComponent(nome)+'&select=id&limit=1');
      }
      ...
      if (existRows && existRows.length && existRows[0].id) {
        // Atualiza (completa sem sobrescrever campos preenchidos)
        delete row.criado_em; delete row.fonte; delete row.status; delete row.temperatura;
        saved = await supaJwtFig('/rest/v1/crm_decisores?id=eq.'+existRows[0].id, {
          method:'PATCH', headers:{'Prefer':'return=representation'}, body:JSON.stringify(row)
        });
        saved = Array.isArray(saved) ? saved[0] : saved;
      } else {
        saved = await supaJwtFig('/rest/v1/crm_decisores', {
          method:'POST', headers:{'Prefer':'return=representation'}, body:JSON.stringify(row)
        });
        saved = Array.isArray(saved) ? saved[0] : saved;
      }
      // Marca como salvo na UI
      setLushaResults(function(prev){return prev.map(function(r2){
        return (r2.firstName===result.firstName&&r2.lastName===result.lastName)?Object.assign({},r2,{saved:true,savedId:saved&&saved.id}):r2;
      });});
      // Atualiza local acc também
      saveAcc(Object.assign({},acc,{decisors: verificados.concat([{
        nome:nome, cargo:result.title||'', email:result.email||'', wa:result.wa||'',
        linkedin_url:result.linkedin_url||'', addedAt:new Date().toLocaleDateString('pt-BR')
      }])}));
    }
    ...
    lushaOpen && React.createElement("div", {
      ...
      style: {
        background: "#0d0d1a", border: "1px solid #2D2D44", borderRadius: 12,
        padding: 28, minWidth: 340, maxWidth: 560, width: "90%", maxHeight: "80vh",
      ...
      React.createElement("div", {style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}},
        ...
        React.createElement("button", {onClick:function(){setLushaOpen(false);setLushaStep('idle');},style:{background:"none",border:"none",color:"#9B9BB4",fontSize:20,cursor:"pointer",lineHeight:1}}, "×")
      ),
      ...
      lushaStep==="select" && React.createElement("div", null,
        React.createElement("div", {style:{fontSize:12,color:"#9B9BB4",marginBottom:12}},
          "Selecione até 5 candidatos para revelar (não gasta crédito ainda):",
        ...
        lushaCandidates.map(function(c, i){
          ...
          return React.createElement("div", {
            ...
            style: {
              border: "1px solid "+(sel?"#818CF8":"#2D2D44"),
              background: sel?"#1a1a3a":"#111120",
              borderRadius: 8, padding: "10px 14px", marginBottom: 8, cursor: "pointer",
              display: "flex", alignItems: "center", gap: 12
            }
          },
            React.createElement("div", {style:{
              width:18,height:18,borderRadius:4,border:"2px solid "+(sel?"#818CF8":"#444"),
              background:sel?"#818CF8":"transparent",flexShrink:0,
        ...
        React.createElement("div", {style:{display:"flex",gap:10,marginTop:18}},
          React.createElement("button", {
            ...
            style:{padding:"9px 20px",borderRadius:7,border:"1px solid #2D2D44",background:"transparent",color:"#9B9BB4",fontSize:12,cursor:"pointer"}
          }, "Cancelar"),
          React.createElement("button", {
            onClick: revealLusha,
            disabled: !lushaSelected.length,
            style:{padding:"9px 20px",borderRadius:7,border:"none",background:lushaSelected.length?"#818CF8":"#333",color:"#fff",fontSize:12,cursor:lushaSelected.length?"pointer":"default",fontWeight:700}
      ...
      lushaStep==="done" && React.createElement("div", null,
        ...
        lushaResults.map(function(r, i){
          return React.createElement("div", {
            ...
            !r.error && !r.saved && React.createElement("button", {
              onClick: function(){saveToSupa(r);},
              style:{marginTop:8,padding:"5px 12px",borderRadius:5,border:"none",background:"#818CF8",color:"#fff",fontSize:11,cursor:"pointer"}
            }, "Salvar em decisores"),
        ...
        React.createElement("button", {
          onClick:function(){setLushaOpen(false);setLushaStep('idle');},
          style:{marginTop:16,padding:"9px 24px",borderRadius:7,border:"none",background:"#2D2D44",color:"#F5F5F5",fontSize:12,cursor:"pointer"}
        }, "Fechar")
      )
    )),
    /*#__PURE__*/React.createElement("div", {
...
function AprovacaoHoje() {
  ...
  var BLOCO_SZ = 10;
  var SEL_FILA = '&select=*,crm_empresas!empresa_id(id,nome,setor),crm_decisores!decisor_id(id,nome,cargo,email,wa,linkedin_url,temperatura,respondeu),crm_agencias!agencia_id(id,nome)';

  function canalQ(c) {
    return c === 'linkedin'
      ? 'canal=in.(linkedin,linkedin_convite,linkedin_mensagem)'
      : 'canal=eq.' + c;
  }
  function load(canalArg, blocoArg) {
    var c = canalArg !== undefined ? canalArg : aba;
    var b = blocoArg !== undefined ? blocoArg : bloco;
    var offset = b * BLOCO_SZ;
  ...
  function loadContadores() {
    var jwt = (window.__supaSession && window.__supaSession.access_token) || SUPA_ANON;
    var hdrs = {apikey:SUPA_ANON, Authorization:'Bearer '+jwt, Prefer:'count=exact'};
    function countC(q) {
      return fetch(SUPA_URL+'/rest/v1/crm_fila?status=eq.rascunho&'+q+'&select=id',
        {method:'HEAD', headers:hdrs})
        .then(function(r){var cr=r.headers.get('content-range');return cr?(parseInt(cr.split('/')[1],10)||0):0;})
    ...
    Promise.all([
      countC('canal=eq.email'),
      countC('canal=eq.whatsapp'),
      countC('canal=in.(linkedin,linkedin_convite,linkedin_mensagem)'),
      ...
      setDbCounts({email:res[0], whatsapp:res[1], linkedin:res[2]});
  ...
  var itensAba = fila||[];
  var totalFila = (dbCounts.email||0)+(dbCounts.whatsapp||0)+(dbCounts.linkedin||0);
  var restantes = Math.max(0, (dbCounts[aba]||0) - (bloco+1)*BLOCO_SZ);
  ...
  return React.createElement("div",{style:{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}},
    ...
    React.createElement("div",{style:{flex:1,overflowY:"auto",padding:"10px 20px"}},
      ...
      // Navegação de blocos
      (dbCounts[aba]||0) > BLOCO_SZ && React.createElement("div",{style:{display:"flex",alignItems:"center",gap:8,padding:"12px 0 4px",borderTop:".5px solid #1A1A2E",marginTop:4}},
        bloco>0 && React.createElement("button",{onClick:function(){var b=bloco-1;setBloco(b);load(aba,b);},style:{...s,fontSize:9,padding:"4px 14px",borderRadius:5,border:".5px solid #2D2D44",background:"transparent",color:"#9B9BB4",cursor:"pointer"}},"← Bloco anterior"),
        React.createElement("span",{style:{...s,fontSize:9,color:"#555",flex:1,textAlign:"center"}},
          "Bloco "+(bloco+1)+" / "+Math.ceil((dbCounts[aba]||0)/BLOCO_SZ)+" · "+(dbCounts[aba]||0)+" itens no total"
        ),
        restantes>0 && React.createElement("button",{onClick:function(){var b=bloco+1;setBloco(b);load(aba,b);},style:{...s,fontSize:9,padding:"4px 14px",borderRadius:5,border:".5px solid rgba(129,140,248,.4)",background:"rgba(129,140,248,.08)",color:"#818CF8",cursor:"pointer",fontWeight:700}},"Próximo bloco ("+restantes+" restantes) →")
      )
    )
  );
}
...
function App() {
  ...
  useEffect(() => {
    function handleAbordagemCopiloto(e) {
      const p = e.detail || {};
      navTo('base', p.agencia_id || null, null);
      if (p.empresa) setTimeout(() => window.dispatchEvent(new CustomEvent('crm:abrirAbordar', { detail: p })), 500);
    }
